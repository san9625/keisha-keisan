...
### 5.4 Rounding Units & Top-Rank Adjustment（丸め単位と最上位負担）
- 丸め単位は {1, 100, 1000} 円から選択できる。デフォルトは 100 円。
- 各ランクの計算では、最上位ランクのみ 1 円刻み、それ以外は選択した丸め単位で切り捨てを行う。
- 差分調整では小数部の大きい・高ランク優先で加算/減算する。どうしても合計が一致しない場合は、最上位ランクの 1 人あたり額を上乗せし、少額のオーバー徴収を許容して合計を収束させる。
